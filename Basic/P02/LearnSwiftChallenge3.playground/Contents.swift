...
class Library {
    ...
    let notExistMessage = "Book doesn't exist"
    ...
    func searchByTitle(_ title:String) -> String {
        ...
        let searchedCatalogueOptional = catalogue.filter{$1.title == title}.first
        guard let searchedCatalogue = searchedCatalogueOptional else { return "Not in catalogue" }
        
        if let person = checkedOutBooks[searchedCatalogue.key], let name = person.name {
